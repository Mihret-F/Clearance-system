...
export default function ApproverDashboard({ user, clearanceRequests, setClearanceRequests }: ApproverDashboardProps) {
  ...
  const [showPasswordChangeDialog, setShowPasswordChangeDialog] = useState(user.isFirstLogin || false)
  ...
  const reportsData = [
    ...
    },
    {
      ...
      data: {
        departments: [
          { name: "Computer Science", approved: 18, rejected: 2, avgTime: "1.2 days" },
  ...
  const getPendingRequests = () => {
    let filtered = clearanceRequests.filter((request) => {
      ...
      const isCurrentApprover =
        request.currentApprover === user.role || (user.department && request.currentApprover === user.department)
      // Check if the request is pending and hasn't been approved by this role
      const isPending =
        request.status === "Pending" && !request.approvals?.some((approval) => approval.role === user.role)
    ...
    return clearanceRequests.filter((request) =>
      request.approvals?.some(
        (approval) =>
          ...
          (user.department && approval.role === user.department && approval.status === "Approved"),
      ),
    )
  }
    ...
    return clearanceRequests.filter((request) =>
      request.approvals?.some(
        (approval) =>
          ...
          (user.department && approval.role === user.department && approval.status === "Rejected"),
      ),
    )
  }
    ...
    try {
      ...
      const updatedRequests = clearanceRequests.map((req) => {
        if (req.id === selectedRequest.id) {
          ...
          const nextApprover = currentIndex < req.approvalChain.length - 1 ? req.approvalChain[currentIndex + 1] : null
      ...
      const updatedRequests = clearanceRequests.map((req) => {
        if (selectedRequests.includes(req.id)) {
          if (bulkAction === "approve") {
            ...
            const currentIndex = req.approvalChain.indexOf(user.role)
            const nextApprover =
              currentIndex < req.approvalChain.length - 1 ? req.approvalChain[currentIndex + 1] : null
      ...
      setNotifications([
        {
          ...
          message: `You have ${bulkAction === "approve" ? "approved" : "rejected"} ${selectedRequests.length} requests.`,
          time: "Just now",
          read: false,
    ...
    return (
      <div className="space-y-6 p-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Request Review</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Reviewing request {selectedRequest.id} from {selectedRequest.userId}
            </p>
          </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4">
                  <Avatar className="h-16 w-16">
                    <AvatarImage
                      src={`/placeholder.svg?height=64&width=64&text=${selectedRequest.userId?.charAt(0) || "U"}`}
                    />
                    <AvatarFallback>{selectedRequest.userId?.charAt(0) || "U"}</AvatarFallback>
                  </Avatar>
                  <div>
                    <h3 className="text-lg font-medium">{selectedRequest.userId}</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {selectedRequest.department || "Department"} • {selectedRequest.program || "Program"}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{new Date(selectedRequest.submittedAt).toLocaleDateString()}</p>
                  </div>
                  <div>
              ...
              <CardContent>
                {selectedRequest.approvalChain && (
                  <WorkflowProgress
                    ...
                    currentStep={selectedRequest.approvalChain.indexOf(selectedRequest.currentApprover)}
                    rejectedStep={
                      selectedRequest.status === "Rejected"
                        ? selectedRequest.approvalChain.indexOf(selectedRequest.currentApprover)
                        : undefined
                    }
                  />
                )}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(value) => setRequestedDocuments([...requestedDocuments, value])}>
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="no_dues_certificate">No Dues Certificate</SelectItem>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {requestedDocuments.map((doc, index) => (
                        <Badge key={index} variant="secondary" className="gap-2">
                          {doc}
                          <button
                            onClick={() => setRequestedDocuments(requestedDocuments.filter((_, i) => i !== index))}
                ...
                <div className="flex flex-col gap-3 mt-6">
                  {requestedDocuments.length > 0 ? (
                    <Button onClick={handleRequestDocuments} disabled={isProcessing} className="w-full gap-2">
                ...
                {!reviewComment && selectedRequest.status === "Pending" && (
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
                    <AlertCircle className="h-3 w-3 inline mr-1" />A comment is required when rejecting a request
                  </p>
                )}
              </CardContent>
            </Card>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 rounded-md">
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <span className="text-sm text-blue-600 dark:text-blue-400">Request Submitted</span>
                      </div>
        ...
        {showDocumentViewer && (
          <DocumentViewer document={currentDocument} onClose={() => setShowDocumentViewer(false)} />
        )}
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{user.department} Approver Dashboard</h1>
          <p className="text-gray-600 dark:text-gray-400">Welcome, {user.name || user.username}</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 ml-2">
            <Avatar className="h-8 w-8 cursor-pointer" onClick={() => setShowProfileDialog(true)}>
              <AvatarImage src="/placeholder.svg?height=32&width=32&text=DH" alt={user.name || user.username} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{getPendingRequests().length}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{getApprovedRequests().length}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Rejected Requests</p>
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{getRejectedRequests().length}</p>
            </div>
          </div>
        </motion.div>
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Overview of approvals and rejections over time</CardDescription>
                </div>
                ...
                <div>
                  <div className="space-y-6">
                    <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                      <h3 className="text-sm font-medium mb-2">Approval Rate</h3>
                      <div className="flex items-center gap-2 mb-2">
                        <div className="text-2xl font-bold">{analyticsData.approvalRates[dateRange].approved}%</div>
                        <Badge
                          variant="outline"
                      ...
                      <Progress value={analyticsData.approvalRates[dateRange].approved} className="h-2" />
                      <div className="flex justify-between text-xs text-gray-500 mt-1">
                        <span>Approved: {analyticsData.approvalRates[dateRange].approved}%</span>
                        <span>Rejected: {analyticsData.approvalRates[dateRange].rejected}%</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <div className="text-2xl font-bold">{analyticsData.processingTimes[dateRange]} days</div>
                        <Badge
                          variant="outline"
                      ...
                      <div className="text-xs text-gray-500">Average time to process a request</div>
                    </div>

                    <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg">
                      <h3 className="text-sm font-medium mb-2">Total Processed</h3>
                      <div className="text-2xl font-bold">{analyticsData.approvalRates[dateRange].total}</div>
                      <div className="text-xs text-gray-500">Total requests processed in this period</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Requests that need your immediate attention</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {getPendingRequests()
                        ...
                        <div className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/80 transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600 dark:text-gray-400">
                                From: {request.userId} • {new Date(request.submittedAt).toLocaleDateString()}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                ...
                {getPendingRequests().length > 3 && (
                  <div className="mt-4 text-center">
                    <Button variant="link" onClick={() => setActiveTab("pending")} className="gap-1">
                ...
                {getPendingRequests().length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                      You have no pending requests that require your attention at this time.
                    </p>
                  </div>
                )}
              </CardContent>
        ...
        <TabsContent value="pending" className="space-y-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="sm" onClick={() => setShowFilterDialog(true)} className="gap-1">
            ...
            {selectedRequests.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600 dark:text-gray-400">{selectedRequests.length} selected</span>
                <Button variant="outline" size="sm" onClick={() => handleBulkAction("approve")} className="gap-1">
                  <CheckSquare className="h-4 w-4" />
                  Approve Selected
                </Button>
                <Button variant="outline" size="sm" onClick={() => handleBulkAction("reject")} className="gap-1">
            ...
            <div className="lg:col-span-3">
              <Card className="bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700">
                <CardHeader>
                  ...
                  <CardDescription>{getPendingRequests().length} requests awaiting your review</CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-gray-700">
                          <th className="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">
                            <Checkbox
                              checked={
                                selectedRequests.length > 0 && selectedRequests.length === getPendingRequests().length
                              }
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  setSelectedRequests(getPendingRequests().map((req) => req.id))
                                } else {
                                  setSelectedRequests([])
                                }
                              }}
                            ...
                            <td className="px-4 py-4">
                              <div className="flex items-center">
                                <Avatar className="h-8 w-8 mr-2">
                                  <AvatarImage
                                    src={`/placeholder.svg?height=32&width=32&text=${request.userId?.charAt(0) || "U"}`}
                                  />
                                  <AvatarFallback>{request.userId?.charAt(0) || "U"}</AvatarFallback>
                                </Avatar>
                                <div>
        ...
        <TabsContent value="approved" className="space-y-4">
          <Card className="bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700">
            <CardHeader>
              ...
              <CardDescription>History of requests you have approved</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Approved on{" "}
                              {new Date(
                                request.approvals.find((a) => a.role === user.role || a.role === user.department)
                                  .timestamp,
                              ).toLocaleDateString()}
                            </p>
                          </div>
                        ...
                        <Collapsible className="mt-4">
                          ...
                          <CollapsibleContent className="mt-2 space-y-3">
                            <div>
                              ...
                              <p className="text-sm text-gray-600 dark:text-gray-400">{request.userId}</p>
                            </div>
                            ...
                            {request.approvalChain && (
                              <div>
                                ...
                                <WorkflowProgress
                                  ...
                                  currentStep={
                                    request.approvalChain.indexOf(request.currentApprover) >= 0
                                      ? request.approvalChain.indexOf(request.currentApprover)
                ...
                {getApprovedRequests().length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                      You haven't approved any requests yet. They will appear here once you do.
                    </p>
                  </div>
                )}
              </div>
        ...
        <TabsContent value="rejected" className="space-y-4">
          <Card className="bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700">
            <CardHeader>
              ...
              <CardDescription>History of requests you have rejected</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                          ...
                          <CollapsibleContent className="mt-2 space-y-3">
                            ...
                            {request.approvalChain && (
                              <div>
                                ...
                                <WorkflowProgress
                                  steps={request.approvalChain}
                                  currentStep={request.approvalChain.indexOf(request.currentApprover)}
                                  rejectedStep={
                                    request.approvalChain.indexOf(user.role) >= 0
                                      ? request.approvalChain.indexOf(user.role)
                                      : request.approvalChain.indexOf(user.department)
                                  }
                                />
                              </div>
                            )}
                ...
                {getRejectedRequests().length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                      You haven't rejected any requests yet. They will appear here once you do.
                    </p>
                  </div>
                )}
              </div>
        ...
        <TabsContent value="reports" className="space-y-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">View and generate reports on clearance request data</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <CardFooter className="flex justify-between">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600 dark:text-gray-400">Approvals</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Rejections</span>
                  </div>
                </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">{analyticsData.approvalRates[dateRange].total}</span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">{analyticsData.processingTimes[dateRange]} days</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
      ...
      <Dialog open={showReviewDialog} onOpenChange={setShowReviewDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Review the request details and make your decision.</DialogDescription>
          </DialogHeader>
            ...
            {selectedRequest && (
              <>
                <div className="grid gap-4">
                  ...
                  <div>
                    <Label>Comment {bulkAction === "reject" && <span className="text-red-500">*</span>}</Label>
                    <Textarea
                      value={reviewComment}
                      onChange={(e) => setReviewComment(e.target.value)}
                    ...
                    {bulkAction === "reject" && !reviewComment && (
                      <p className="text-xs text-red-500 mt-1">A comment is required for rejections</p>
                    )}
                  </div>
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" onClick={() => setShowReviewDialog(false)} disabled={isProcessing}>
                    Cancel
                  </Button>
                  {requestedDocuments.length > 0 ? (
                    <Button onClick={handleRequestDocuments} disabled={isProcessing} className="gap-2">
                    ...
                    <>
                      ...
                      <Button variant="default" onClick={handleApprove} disabled={isProcessing} className="gap-2">
                        <CheckCircle className="h-4 w-4" />
                        Approve
                      </Button>
                    </>
      ...
      {/* Document Viewer Dialog */}
      {showDocumentViewer && <DocumentViewer document={currentDocument} onClose={() => setShowDocumentViewer(false)} />}

      {/* Bulk Action Dialog */}
      <Dialog open={showBulkActionDialog} onOpenChange={setShowBulkActionDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{bulkAction === "approve" ? "Bulk Approve Requests" : "Bulk Reject Requests"}</DialogTitle>
            <DialogDescription>
              You are about to {bulkAction === "approve" ? "approve" : "reject"} {selectedRequests.length} requests.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                placeholder={`Add a comment for all ${bulkAction === "approve" ? "approvals" : "rejections"}...`}
                className="mt-1"
              />
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setShowBulkActionDialog(false)} disabled={isProcessing}>
                Cancel
              </Button>
              <Button
                variant={bulkAction === "approve" ? "default" : "destructive"}
                onClick={executeBulkAction}
                disabled={isProcessing || (bulkAction === "reject" && !bulkComment)}
                className="gap-2"
              >
                {isProcessing ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ...
                {bulkAction === "approve" ? "Approve" : "Reject"} {selectedRequests.length} Requests
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={showFilterDialog} onOpenChange={setShowFilterDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Apply filters to narrow down the request list</DialogDescription>
          </DialogHeader>
            ...
            <div>
              ...
              <Select value={filterRequestType} onValueChange={setFilterRequestType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Graduation Clearance">Graduation Clearance</SelectItem>
                  <SelectItem value="Transfer Clearance">Transfer Clearance</SelectItem>
                  <SelectItem value="Library Clearance">Library Clearance</SelectItem>
                  <SelectItem value="Leave Clearance">Leave Clearance</SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center">
              <Avatar className="h-20 w-20 mb-4">
                <AvatarImage src="/placeholder.svg?height=80&width=80&text=DH" alt={user.name || user.username} />
                <AvatarFallback>{user.name?.charAt(0) || user.username.charAt(0)}</AvatarFallback>
              </Avatar>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input defaultValue={user.name || user.username} className="mt-1" />
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">Toggle between light and dark themes</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">Play sounds for new notifications</span>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">Enhance your account security</span>
                </div>
                <Button variant="outline" size="sm">
                  Enable
                </Button>
              ...
              <div className="pt-2">
                <Button variant="outline" className="w-full" onClick={() => setShowPasswordChangeDialog(true)}>
                  Change Password
                </Button>
              </div>
            </div>
      ...
      <Dialog open={showNotificationPanel} onOpenChange={setShowNotificationPanel}>
        <DialogContent className="max-w-md">
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    >
                      ...
                      {notification.type === "error" && <XCircle className="h-5 w-5 text-red-600 dark:text-red-400" />}
                      {notification.type === "warning" && (
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" className="gap-1" onClick={markAllNotificationsAsRead}>
              <CheckCircle className="h-4 w-4" />
              Mark All as Read
            </Button>
      ...
      <Dialog open={showInactivityDialog} onOpenChange={setShowInactivityDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>You've been inactive for a while. Would you like to stay logged in?</DialogDescription>
          </DialogHeader>

          <div className="py-4 text-center">
            <p className="text-lg font-medium">
              Logging out in <span className="text-red-600 dark:text-red-400">{inactivityCountdown}</span> seconds
            </p>
            <Progress value={(inactivityCountdown / 60) * 100} className="mt-4" />
          </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{user.isFirstLogin ? "Change Default Password" : "Change Password"}</DialogTitle>
            <DialogDescription>
              {user.isFirstLogin
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Add your email address for notifications and account recovery.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showReportDialog} onOpenChange={setShowReportDialog}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Generated on {currentReport ? new Date(currentReport.date).toLocaleDateString() : ""}
            </DialogDescription>
          </DialogHeader>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Approved:</span>
                        <span className="font-medium">{currentReport.data.approved}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Rejected:</span>
                        <span className="font-medium">{currentReport.data.rejected}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Pending:</span>
                        <span className="font-medium">{currentReport.data.pending}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total:</span>
                        <span className="font-medium">
                          {currentReport.data.approved + currentReport.data.rejected + currentReport.data.pending}
                        </span>
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="flex flex-col items-center justify-center h-32">
                      <div className="text-3xl font-bold">{currentReport.data.avgProcessingTime}</div>
                    ...
                    {currentReport.data.departments.map((dept, index) => (
                      <div key={index} className="border-b pb-3 last:border-0">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <Badge variant={dept.approved > dept.rejected * 3 ? "success" : "default"}>
                            {dept.approved > dept.rejected * 3 ? "Excellent" : "Good"}
                          </Badge>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-sm">
                          <div>
                            <span className="text-gray-500">Approved:</span> {dept.approved}
                          </div>
                          <div>
                            <span className="text-gray-500">Rejected:</span> {dept.rejected}
                          </div>
                          <div>
                            <span className="text-gray-500">Avg Time:</span> {dept.avgTime}
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="h-80 w-full">
                    <ChartContainer config={{}}>
                      <Chart>
                        <ResponsiveContainer width="100%" height="100%">
                          <RechartsLineChart
                            data={currentReport.data.months.map((month, index) => ({
                              name: month,
                              approvals: currentReport.data.approvals[index],
                              rejections: currentReport.data.rejections[index],
                            }))}
                          ...
                          >
                            ...
                            <RechartsTooltip />
                            <Legend />
                            <Line type="monotone" dataKey="approvals" name="Approvals" stroke="#4ade80" />
                            <Line type="monotone" dataKey="rejections" name="Rejections" stroke="#f87171" />
                          </RechartsLineChart>
                        </ResponsiveContainer>
                      </Chart>
                    </ChartContainer>
